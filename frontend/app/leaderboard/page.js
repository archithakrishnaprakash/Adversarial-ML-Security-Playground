...
const SORT_OPTIONS = [
  { value: "robustness_score", label: "Robustness score (high to low)" },
...
export default function LeaderboardPage() {
  ...
  const maxScore = Math.max(1, ...rows.map((r) => r.robustness_score ?? 0));
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Ranks every model that's had at least one robustness evaluation run against it (Robustness Report page). Train and evaluate more models to grow this list."
      />
                ...
                <div className="w-40">
                  <div className="h-2 bg-panel2 border border-border">
                    <div
                      ...
                      style={{ width: `${((row.robustness_score ?? 0) / maxScore) * 100}%` }}
                    />
                  </div>
                </div>
