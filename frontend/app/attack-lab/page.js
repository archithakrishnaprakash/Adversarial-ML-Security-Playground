...
const CAPABILITY_OPTIONS = [
  { value: "white_box", label: "White-box (weights + gradients)" },
  { value: "gray_box", label: "Gray-box (prediction probabilities only)" },
  { value: "black_box", label: "Black-box (prediction API only)" },
];

export default function AttackLabPage() {
  const [models, setModels] = useState([]);
  const [modelId, setModelId] = useState(null);
  const [capability, setCapability] = useState("white_box");
  const [threatMatrix, setThreatMatrix] = useState(null);
  ...
  useEffect(() => {
    ...
    api.getThreatModelMatrix().then((r) => setThreatMatrix(r.matrix));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const selectedModel = models.find((m) => m.model_id === modelId);
  const capabilityRules = threatMatrix?.[capability];

  // when the threat model changes, jump off an attack that's no longer
  // applicable rather than silently letting the request 400
  useEffect(() => {
    if (!capabilityRules) return;
    if (capabilityRules[attack] && !capabilityRules[attack].applicable) {
      const firstApplicable = ATTACK_OPTIONS.find((o) => capabilityRules[o.value]?.applicable);
      if (firstApplicable) setAttack(firstApplicable.value);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [capability, threatMatrix]);
  ...
  return (
    <div>
      <PageHeader
        eyebrow="Step 2"
        title="Attack Lab"
        description="Pick a threat model, choose an attack, and watch the original input, its perturbation, and the resulting prediction side by side."
      />
      ...
      <div className="grid grid-cols-3 gap-6 mb-8">
        <div className="col-span-1 space-y-4">
          ...
          <Panel eyebrow="Threat model" title="Attacker capability">
            <Select
              value={capability}
            ...
            <p className="text-[11px] text-muted leading-relaxed mt-2">
              {capability === "white_box" &&
                "Full access to model weights and gradients — every gradient attack applies directly."}
              {capability === "gray_box" &&
                "Only prediction probabilities are exposed — gradient attacks run against a surrogate trained to mimic the target, then transfer."}
              {capability === "black_box" &&
                "Only a prediction API is exposed — gradient attacks (FGSM/PGD/DeepFool) don't apply at all. Use the Black-Box Query Lab below instead."}
            </p>
          </Panel>
          ...
          <Panel eyebrow="Configuration" title="Attack">
            <div className="space-y-4">
              <Select
                ...
                value={attack}
                onChange={setAttack}
                options={ATTACK_OPTIONS.map((o) => {
                  const applicable = capabilityRules ? capabilityRules[o.value]?.applicable : true;
                  return { value: o.value, label: applicable ? o.label : `${o.label} — not applicable` };
                })}
              />
              {capabilityRules && !capabilityRules[attack]?.applicable && (
                <p className="text-[11px] text-red leading-relaxed">
                  {capabilityRules[attack].rationale}
                </p>
              )}
              {attack !== "deepfool" && (
                <div>
              ...
              <Button
                variant="danger"
                onClick={handleRun}
                disabled={!modelId || running || (capabilityRules && !capabilityRules[attack]?.applicable)}
                className="w-full"
              >
      ...
      <BlackBoxLab models={models} />
    </div>
  );
}

function BlackBoxLab({ models }) {
  const [modelId, setModelId] = useState(null);
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (method === "transfer") payload.n_queries = Number(budget);
      else payload.max_queries_per_sample = Number(budget);

      const res = await api.runBlackBoxAttack(payload);
      setResult(res);
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  return (
    <Panel
      eyebrow="Pure black-box"
      title="Black-Box Query Lab"
      right={<Pill tone="amber">no gradients used, ever</Pill>}
    >
      <p className="text-sm text-muted mb-4 max-w-2xl">
        Attacks the model using only its prediction API — the realistic case for an attacker who
        never sees weights or gradients. <span className="text-ink">Transfer</span> spends its
        query budget training a substitute model and attacking that; <span className="text-ink">
        query</span> searches directly against the target, querying it for every candidate step.
      </p>
      <div className="grid grid-cols-3 gap-4 mb-4">
        <ModelPicker models={models} value={modelId} onChange={setModelId} />
        <div className="space-y-4">
          <Select
            ...
            onChange={setMethod}
            options={[
              { value: "transfer", label: "Transfer (surrogate)" },
              { value: "query", label: "Query (zeroth-order search)" },
            ]}
          />
          <div>
            <div className="flex justify-between items-baseline mb-1.5">
        ...
        <div className="space-y-4">
          <div>
            <div className="flex justify-between items-baseline mb-1.5">
              <span className="font-display text-[10px] tracking-widest text-muted uppercase">
                {method === "transfer" ? "Surrogate training queries" : "Max queries per sample"}
              </span>
              <span className="font-mono text-sm text-ink">{budget}</span>
            </div>
            <input
              type="range"
              min="20"
          ...
          <Button onClick={handleRun} disabled={!modelId || running} className="w-full">
            {running ? "Querying…" : "Run black-box attack"}
          </Button>
        </div>
      </div>
      {error && <p className="text-xs text-red font-mono mb-2">{error}</p>}
      {result && (
        <div className="grid grid-cols-5 gap-4 pt-4 border-t border-border">
          ...
          <MetricStat
            ...
            value={`${Math.round(result.adversarial_accuracy * 100)}%`}
            tone="red"
          />
          <MetricStat
            label="Success rate"
            value={`${Math.round(result.attack_success_rate * 100)}%`}
            tone="amber"
          />
          <MetricStat label="Queries used" value={result.queries_used} />
          <MetricStat label="Queries / sample" value={result.queries_per_sample} />
        </div>
      )}
    </Panel>
  );
