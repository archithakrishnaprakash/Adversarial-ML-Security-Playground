  ...
  return (
    <div>
      <PageHeader
        ...
        description="Run a batch evaluation across the model's held-out test set. Standard metrics (robust accuracy, ASR, L1/L2/L∞, confidence shift) come first — the Aegis Robustness Index is a single composite number for at-a-glance comparison, not a substitute for them."
      />
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 space-y-4">
          ...
          {result && (
            <>
              <Panel eyebrow="Standard metrics" title="Robustness by attack">
                  ...
                  <table className="w-full text-sm font-mono">
                    <thead>
                      <tr className="text-left text-muted text-[11px] uppercase tracking-wide">
                        ...
                        <th className="pb-2 font-display font-normal">Robust acc.</th>
                        <th className="pb-2 font-display font-normal">Degradation</th>
                        <th className="pb-2 font-display font-normal">ASR</th>
                        <th className="pb-2 font-display font-normal">L1</th>
                      ...
                      {Object.entries(result.attacks).map(([name, v]) => (
                        <tr key={name} className="border-t border-border">
                          ...
                          <td className="py-1.5 text-muted">
                            {v.accuracy_degradation != null
                              ? `${Math.round(v.accuracy_degradation * 100)}%`
                              : "—"}
                          </td>
                          ...
                          <td className="py-1.5 text-muted">{v.l1_norm ?? "—"}</td>
              ...
              <Panel
                eyebrow="Composite (project metric)"
                title="Aegis Robustness Index"
                right={
              ...
              >
                <div className="grid grid-cols-2 gap-4 mb-3">
                  <MetricStat
                    label="Aegis Robustness Index"
                    value={result.aegis_robustness_index ?? result.robustness_score}
                    suffix="/100"
                    tone={scoreTone(result.aegis_robustness_index ?? result.robustness_score)}
                  />
                  <div className="h-2 bg-panel2 border border-border self-center">
                    <div
                      className={`h-full ${
                        (result.aegis_robustness_index ?? result.robustness_score) >= 70
                          ? "bg-green"
                          : (result.aegis_robustness_index ?? result.robustness_score) >= 40
                          ? "bg-amber"
                          : "bg-red"
                      }`}
                      style={{ width: `${result.aegis_robustness_index ?? result.robustness_score}%` }}
                    />
                  </div>
                </div>
                <p className="text-[11px] text-muted leading-relaxed">
                  {result.aegis_robustness_index_note}
                </p>
              </Panel>
            </>
          )}
