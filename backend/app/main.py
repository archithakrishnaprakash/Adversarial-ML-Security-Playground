...
from app.evaluation.robustness_matrix import compute_robustness_matrix
...
app = FastAPI(title="AegisML — Adversarial ML Security Assessment Framework", version="2.0.0")

# Allowed origins for the browser-facing frontend. Defaults to "*" (any
# origin) to preserve local-dev behavior exactly as before — set the
# ALLOWED_ORIGINS env var to a comma-separated list of real origins (e.g.
# "https://your-frontend-domain.com") once this is deployed publicly, so
# only your own frontend can call the API from a browser.
...
def datasets():
    return {
        "datasets": [
            ...
            {
                "id": "cyber",
                "name": "Network Intrusion — Synthetic (quick-start)",
                "type": "tabular",
                "num_classes": 2,
                "class_names": ["BENIGN", "ATTACK"],
                "valid_models": VALID_MODEL_TYPES_BY_DATASET["cyber"],
                "available": True,
                "note": "Generated, not downloaded — deterministic and fast, but not a substitute "
                        "for evaluating against real traffic. Use 'real_ids' for that.",
            },
            {
                "id": "real_ids",
                "name": "Network Intrusion — UNSW-NB15 (real)",
                "type": "tabular",
                "num_classes": 2,
                "class_names": ["BENIGN", "ATTACK"],
                "valid_models": VALID_MODEL_TYPES_BY_DATASET["real_ids"],
                "available": dataset_is_available("real_ids"),
                "note": None if dataset_is_available("real_ids") else
                        "Not available yet — download UNSW-NB15 and set UNSW_NB15_DATA_DIR. "
                        "See README section 5b.",
            },
        ],
...
@app.post("/api/robustness/matrix")
def api_robustness_matrix(req: RobustnessMatrixRequest):
    """The systematic-benchmark view: trains every requested model
    architecture and evaluates every requested attack at one epsilon,
    returning a models x attacks robust-accuracy table in one call instead
    of running Robustness Report once per model."""
    try:
        result = compute_robustness_matrix(
            req.dataset, req.model_types, req.attacks,
            epsilon=req.epsilon, n_samples=req.n_samples, seed=req.seed,
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))

    log_experiment("robustness_matrix", req.dataset, ",".join(req.model_types), "multiple", result,
                    epsilon=req.epsilon)
    return result
