...
def perturbation_norms(X: np.ndarray, X_adv: np.ndarray, l0_threshold: float = 1e-4) -> dict:
    ...
    `l0_threshold`), L2 (Euclidean norm per sample, averaged), and L-infinity
    (max absolute change per sample, averaged) perturbation norms.
    """
    ...
    return {"l0": round(l0, 4), "l2": round(l2, 4), "linf": round(linf, 4)}
