...
from app.evaluation.robustness_matrix import compute_robustness_matrix  # noqa: E402
...
def test_robustness_evaluation_includes_lp_norms():
    ...
    for key in ("l0_norm", "l1_norm", "l2_norm", "linf_norm", "confidence_shift"):
        assert key in fgsm_result


def test_robustness_evaluation_includes_aegis_robustness_index():
    model = train_model("cyber", "logistic_regression")
    result = evaluate_robustness(model, ["fgsm", "random_noise"], epsilon=0.1, n_samples=30)
    assert result["aegis_robustness_index"] == result["robustness_score"]
    assert "aegis_robustness_index_note" in result
    assert result["mean_robust_accuracy"] is not None
    assert result["mean_accuracy_degradation"] is not None


def test_robustness_matrix_shape_and_bounds():
    result = compute_robustness_matrix(
        "cyber",
        model_types=["logistic_regression", "random_forest"],
        attacks=["fgsm", "random_noise"],
        epsilon=0.1,
        n_samples=30,
    )
    assert set(result["model_types"]) == {"logistic_regression", "random_forest"}
    for model_type in result["model_types"]:
        row = result["matrix"][model_type]
        assert set(row.keys()) == {"fgsm", "random_noise"}
        for acc in row.values():
            assert acc is None or 0.0 <= acc <= 1.0
        assert 0.0 <= result["clean_accuracy"][model_type] <= 1.0


def test_robustness_matrix_rejects_oversized_grid():
    with pytest.raises(ValueError):
        compute_robustness_matrix(
            "cyber",
            model_types=["logistic_regression"] * 7,  # exceeds MAX_MODELS
            attacks=["fgsm"],
        )


def test_robustness_matrix_rejects_empty_inputs():
    with pytest.raises(ValueError):
        compute_robustness_matrix("cyber", model_types=[], attacks=["fgsm"])
